...
public class BugSearch implements BugzillaMethod {
	...
	public enum SearchLimiter {
		...
		/**
		 * The resolution field, if the bug's status is closed. You can search for all
		 * open bugs by searching for a blank resolution.
		 */
	 ...
	 * 
	 * @param queries One or more {@code SearchQuery} objects to narrow the search
	 *                by.
	 */
//	public ExtendedBugSearch(List<SearchQuery> queries) {
//		if(queries.size() == 0) { throw new IllegalArgumentException("At least one search query is required"); }
//		
...
//	}
	...
	public BugSearch(Map<Object, Object> queryMap) {
		if (queryMap.size() == 0) {
	...
	public List<Bug> getSearchResults() {
		...
		/*
		 * The following is messy, but necessary due to how the returned XML document
		 * nests Maps.
		 */
		if (hash.containsKey("bugs")) {
			...
			for (Object o : bugs) {
				...
				// Handle version property for older Bugzillas which did not include it in the
				// public portion of the hash
	...
	public static class SearchQuery {
		 ...
		 *                matched, or not at all -- Bugzilla does not perform substring
		 *                matching.
		 */
